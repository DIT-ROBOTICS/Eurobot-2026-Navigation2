...
void SimaLayer::onInitialize() {
  ...
  declareParameter("rival_inscribed_radius",
                   rclcpp::ParameterValue(0.22 * kSimaSizeScale));
  ...
  declareParameter("wandering_cost_scaling_factor",
  ...
  declareParameter("offset_vel_factor_weight_statistic",
                   rclcpp::ParameterValue(0.42));
  declareParameter("expand_vel_factor_weight_statistic",
                   rclcpp::ParameterValue(0.20));
  declareParameter("offset_vel_factor_weight_localization",
                   rclcpp::ParameterValue(0.42));
  declareParameter("expand_vel_factor_weight_localization",
                   rclcpp::ParameterValue(0.20));
  ...
  node->get_parameter(name_ + ".auto_reset_with_timeout",
                      auto_reset_with_timeout_);
  node->get_parameter(name_ + ".reset_timeout_threshold",
                      reset_timeout_threshold_);
  node->get_parameter(name_ + ".robot_inscribed_radius",
                      robot_inscribed_radius_);
  node->get_parameter(name_ + ".external_rival_data_path",
                      external_rival_data_path_);
  node->get_parameter(name_ + ".rival_inscribed_radius",
                      rival_inscribed_radius_);
  node->get_parameter(name_ + ".halted_inflation_radius",
                      halted_inflation_radius_);
  node->get_parameter(name_ + ".wandering_inflation_radius",
                      wandering_inflation_radius_);
  node->get_parameter(name_ + ".moving_inflation_radius",
                      moving_inflation_radius_);
  node->get_parameter(name_ + ".unknown_inflation_radius",
                      unknown_inflation_radius_);
  node->get_parameter(name_ + ".halted_cost_scaling_factor",
                      halted_cost_scaling_factor_);
  node->get_parameter(name_ + ".wandering_cost_scaling_factor",
                      wandering_cost_scaling_factor_);
  node->get_parameter(name_ + ".moving_cost_scaling_factor",
                      moving_cost_scaling_factor_);
  node->get_parameter(name_ + ".unknown_cost_scaling_factor",
                      unknown_cost_scaling_factor_);
  ...
  node->get_parameter(name_ + ".offset_vel_factor_weight_statistic",
                      offset_vel_factor_weight_statistic_);
  node->get_parameter(name_ + ".expand_vel_factor_weight_statistic",
                      expand_vel_factor_weight_statistic_);
  node->get_parameter(name_ + ".offset_vel_factor_weight_localization",
                      offset_vel_factor_weight_localization_);
  node->get_parameter(name_ + ".expand_vel_factor_weight_localization",
...
void SimaLayer::updateBounds(double /*robot_x*/, double /*robot_y*/,
...
void SimaLayer::updateCosts(nav2_costmap_2d::Costmap2D &master_grid,
...
void SimaLayer::handleSetMode(
  ...
  response->message = mode_param_ ? "SimaLayer is in shrink mode"
...
void SimaLayer::updateRadius() {
  ...
  if (!external_rival_data_path_.empty()) {
    try {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("SimaLayer"),
...
void SimaLayer::fieldExpansion(const SimaAgentState &agent) {
  ...
  case RivalState::HALTED:
    expandPointWithCircle(agent.x, agent.y, nav2_costmap_2d::MAX_NON_OBSTACLE,
                          halted_inflation_radius_, halted_cost_scaling_factor_,
                          rival_inscribed_radius_);
    break;
  case RivalState::MOVING:
    expandPointWithCircle(agent.x, agent.y, nav2_costmap_2d::MAX_NON_OBSTACLE,
                          halted_inflation_radius_, halted_cost_scaling_factor_,
                          rival_inscribed_radius_);
    expandLine(agent, nav2_costmap_2d::MAX_NON_OBSTACLE,
               moving_inflation_radius_, moving_cost_scaling_factor_,
               rival_inscribed_radius_, max_extend_length_);
    break;
...
void SimaLayer::expandPointWithCircle(double x, double y, double max_cost,
  ...
  for (double current_x = min_x; current_x <= max_x; current_x += resolution_) {
    const double half_width_sq = inflation_radius * inflation_radius -
                                 std::pow(std::fabs(current_x - x), 2);
    ...
    for (double current_y = min_y; current_y <= max_y;
      ...
      if (distance > inscribed_radius) {
        cost = std::ceil(252.0 * std::exp(-cost_scaling_factor *
      ...
      if (getCost(mx, my) != nav2_costmap_2d::NO_INFORMATION) {
        setCost(mx, my,
                std::max(static_cast<unsigned char>(cost), getCost(mx, my)));
      } else {
...
void SimaLayer::expandLine(const SimaAgentState &agent, double max_cost,
  ...
  const double position_offset =
      std::max(agent.distance - safe_distance_, 0.0) * vel_factor *
      offset_vel_factor_weight_localization_;
  ...
  const double scaled_extend = extend_length * vel_factor *
                               expand_vel_factor_weight_localization_ /
                               distance;
  ...
  if (goal_steps <= 0) {
    if (agent.distance < 0.75) {
      expandPointWithCircle(start_x, start_y, max_cost, inflation_radius,
  ...
  for (int i = 0; i < goal_steps; ++i) {
    ...
    expandPointWithCircle(start_x + mark_x, start_y + mark_y, max_cost,
                          current_inflation_radius, cost_scaling_factor,
                          current_inscribed_radius);
    current_inscribed_radius *= inscribed_radius_rate_;
    current_inflation_radius *= inflation_radius_rate_;
  }
}

void SimaLayer::logStateChange(std::size_t /*index*/,
                               RivalState /*new_state*/) {}
