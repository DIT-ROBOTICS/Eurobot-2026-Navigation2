  ...
  std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap)
{
  ...
  // check if the costmap has an inflation layer by scanning the layered costmap plugins
  ...
  for (const auto & plugin : *plugins) {
    auto p = std::dynamic_pointer_cast<nav2_costmap_2d::InflationLayer>(plugin);
    if (!p) { continue; }
    if (inflation_layer_name_.empty() || p->getName() == inflation_layer_name_) {
      inflation_layer = p;
      break;
    }
  }
